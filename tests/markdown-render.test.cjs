...
test('markdown regression: first images are eager and later images lazy', () => {
  const input = [
    '![首图](https://example.com/first.png)',
    '',
    '![第二张](https://example.com/second.png)',
    '',
    '![第三张](https://example.com/third.png)',
    '',
    '![第四张](https://example.com/fourth.png)',
  ].join('\n');
  const html = renderMarkdown(input).__html;

  assert.match(html, /<img alt="首图" src="https:\/\/example\.com\/first\.png" loading="eager" decoding="async" fetchpriority="high"/);
  assert.match(html, /<img alt="第二张" src="https:\/\/example\.com\/second\.png" loading="eager" decoding="async"/);
  assert.match(html, /<img alt="第三张" src="https:\/\/example\.com\/third\.png" loading="eager" decoding="async"/);
  assert.match(html, /<img alt="第四张" src="https:\/\/example\.com\/fourth\.png" loading="lazy" decoding="async"/);
});
...
test('markdown detail preset: upgraded section headings and unordered list items', () => {
  ...
  const imageClass = html.match(/<img alt="内容配图" src="https:\/\/example\.com\/image\.png" loading="eager" decoding="async" fetchpriority="high" class="([^"]+)" \/>/)?.[1] || '';
