...
import { Search, X, History, Flame, Star, Download } from 'lucide-react';
import { cn } from '@/lib/utils';
import type { SearchResult, Game } from '@/types';
import { MOCK_SEARCH_HISTORY, MOCK_SEARCH_RECOMMENDATIONS, MOCK_GAMES } from '@/lib/constants';
                ...
                <div className="space-y-6 animate-in fade-in-50">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 px-2">
                            {MOCK_SEARCH_RECOMMENDATIONS.map(item => (
                                <Link href={`/app/${item.pkg || item.id}`} key={item.id} onClick={() => setIsOpen(false)} className="block">
