...
function extractLeadText(post: ApiDynamicPost, maxLength = 160): string {
  const title = String(post.title || '').trim();
  const summary = String(post.summary || '').trim();
  const rawContent = String(post.content || post.body || '').trim();

  const candidates: string[] = [];
  if (rawContent) candidates.push(extractPlainText(rawContent));
  if (summary) candidates.push(summary);
  if (title) candidates.push(title);

  if (candidates.length === 0) return '分享了一条动态';
  for (const text of candidates) {
    if (isDuplicateText(text, title) || isDuplicateText(text, summary)) continue;
    if (text.length > maxLength) return `${text.slice(0, maxLength)}...`;
    return text;
  }

  if (summary) return summary.length > maxLength ? `${summary.slice(0, maxLength)}...` : summary;
  ...
  return (
    ...
    <div className="overflow-hidden rounded-2xl border border-[#e5e7eb] bg-white dark:border-[#2a3442] dark:bg-[#111824]">
      {posts.map((post, index) => {
        ...
        const leadText = extractLeadText(post, 260);
        ...
        return (
          ...
          >
            ...
            <Link href={`/community/post/${postId}`} className="block">
              ...
              <p className={cn('text-[14px] leading-[1.62] text-[#5a6270] dark:text-[#9ca6b8] sm:text-[15px] sm:line-clamp-8', hasMeaningfulTitle ? 'mt-1 line-clamp-6' : 'line-clamp-8')}>
                {leadText}
              </p>
              {hashtags.length > 0 ? (
