  ...
  return {
    ...
    description: buildSeoDescription(
      textOrFallback(page?.seo_description, defaults.description),
        [
        key === 'contact'
          ? '可提交页面链接、应用包名、问题描述和截图，便于快速定位内容与资源问题；反馈内容会由运营人员核对后处理，并尽量同步处理进展。我们也会持续检查资源可用性、页面信息和用户反馈，帮助改进站点服务质量'
          : '',
        key === 'privacy_policy'
          ? '说明信息收集、使用、存储和用户权利等站点数据处理范围；同时介绍账号、反馈、访问统计和外部服务相关的数据使用边界。阅读后可了解站点如何保护信息、处理请求以及响应隐私相关问题，并查看适用的用户选择与联系渠道'
          : '',
        key === 'about'
